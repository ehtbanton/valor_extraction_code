...
def ask_gemini(agent: genai.GenerativeModel, prompt: str, file_paths: Optional[List[str]] = None) -> str:
    """
    Sends a prompt and a list of files to the initialized Gemini agent and gets a response.

    Args:
        agent (genai.GenerativeModel): The initialized generative model agent.
        prompt (str): The text prompt to send to the model.
        file_paths (Optional[List[str]]): A list of paths to the files to be sent. Defaults to None.
    ...
    # --- New: Prepare content list with text prompt and files ---
    content = [prompt]
    uploaded_files = []
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            try:
                ...
                # If a file fails to upload, we'll notify the user and skip it.
